...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        ////        bgAlphaView.backgroundColor = [UIColor blueColor];
        //        [self addSubview:bgAlphaView];
        NSInteger width = [UIScreen mainScreen].bounds.size.width;
        NSInteger height = [UIScreen mainScreen].bounds.size.height;
        //内容区的背景
        _contentBgView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, width, height)];
        ...
        //        leftImage.frame = CGRectMake(CGRectGetWidth(_contentBgView.frame)/5.0 - 8 - 10, (42-13)/2.0, 8, 13);
        leftImage.frame = CGRectMake(width / 4.0,(42-13)/2.0, 8, 13);
        ...
        rightImage.frame = CGRectMake(width / 4.0 * 3.0, (42-13)/2.0, 8, 13);
        
        _titleLab = [[UILabel alloc] initWithFrame:CGRectMake((width - 100)/2.0 , 0, 100 , 42)];
        ...
        [_backBtn setFrame:CGRectMake((width - 150) / 2, CGRectGetMaxY(_dateBgView.frame) +50, 150, 30)];
        [_backBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [_backBtn.titleLabel setFont:[UIFont systemFontOfSize:14]];
        [_backBtn setBackgroundImage:[[UIImage imageNamed:@"b_com_bt_blue_normal"] stretchableImageWithLeftCapWidth:15 topCapHeight:10] forState:UIControlStateNormal];
        [_backBtn setBackgroundImage:[[UIImage imageNamed:@"b_com_bt_blue_normal"] stretchableImageWithLeftCapWidth:15 topCapHeight:10] forState:UIControlStateSelected];
        [_backBtn setBackgroundImage:[[UIImage imageNamed:@"com_bt_gray_normal"] stretchableImageWithLeftCapWidth:15 topCapHeight:10] forState:UIControlStateDisabled];
        [_backBtn addTarget:self action:@selector(_doneBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [_backBtn setTitle:@"确定" forState:UIControlStateNormal];
        [_contentBgView addSubview:_backBtn];
...
- (void)showDateView {
    ...
    for(int i = startDayIndex; i < kTotalNum;i++)
    {
        ...
        //        NSLog(@"startDayIndex:%f",startDayIndex);
